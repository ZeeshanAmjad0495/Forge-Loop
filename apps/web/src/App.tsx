...
function incidentSeverityStyle(s: IncidentSeverity | null | undefined): React.CSSProperties {
  const map: Record<string, [string, string, string]> = {
  ...
  const [color, background, border] = map[s || 'unknown'] || map.unknown
  return {
    display: 'inline-block',
    padding: '1px 6px',
    borderRadius: 4,
...
function incidentStatusStyle(s: IncidentStatus | null | undefined): React.CSSProperties {
  const map: Record<string, [string, string, string]> = {
  ...
  const [color, background, border] = map[s || 'reported'] || map.reported
  return {
    display: 'inline-block',
    padding: '1px 6px',
    borderRadius: 4,
...
function IncidentsPanel({ projectId }: { projectId: string }) {
  const [open, setOpen] = useState(false)
...
function memoryCandidateStatusStyle(s: MemoryCandidateStatus): React.CSSProperties {
  const map: Record<string, [string, string, string]> = {
  ...
  const [color, background, border] = map[s] || map.proposed
  return {
    display: 'inline-block',
    padding: '1px 6px',
    borderRadius: 4,
...
const MEMORY_TYPES: MemoryCandidateMemoryType[] = [
...
const LEARNING_SOURCE_TYPES: MemoryCandidateSourceType[] = [
  'ci_analysis',
  'incident_analysis',
  'pr_review',
  'check_run',
...
function MemoryCandidatesPanel({ projectId }: { projectId: string }) {
  const [open, setOpen] = useState(false)
