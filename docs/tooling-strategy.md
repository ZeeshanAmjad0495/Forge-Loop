...
Local git branch workflow (Task 37) is implemented as a narrow, allow-listed git boundary. ForgeLoop can inspect git state, create ForgeLoop-scoped local branches (`forgeloop/dev-task/{id}` etc.), and — when both `GIT_WORKFLOW_ENABLED` and `GIT_COMMIT_ENABLED` are set and an approval exists — make a single local commit per call. The service routes every invocation through one `_run_git` helper that uses `subprocess.run(shell=False, env={"PATH": …})`, enforces a top-level argv allow-list (`rev-parse`, `status`, `diff`, `switch`, `add`, `commit`, plus the per-invocation `-c user.name=ForgeLoop -c user.email=forgeloop@local` identity), and rejects everything else. A built-in secrets blocklist (`.env*`, `id_rsa`, `*.pem`, `.aws/`, `.ssh/`, `secrets/`) unioned with the safety profile's `blocked_paths` keeps secrets out of commits. Pre-commit hooks are not bypassed.

GitHub draft PR creation (Task 38) extends the boundary by exactly one operation. `POST /pr-drafts/{id}/create-github-draft` publishes one `approved_for_creation` PR draft: it (optionally) pushes the ForgeLoop-scoped local branch via the narrow `git push <remote-or-auth-url> <branch>` form (no flags, no refspec, no force/mirror/tags/upstream), then POSTs to a single GitHub REST endpoint via a stdlib-only `UrllibGitHubClient` that exposes exactly `create_draft_pull_request`. Master switches `GITHUB_INTEGRATION_ENABLED` and `GITHUB_PUSH_ENABLED` are both `false` by default. The PAT is never persisted, never logged, never echoed in API responses, never stored in artifacts; a single `_redact_token` helper scrubs stdout/stderr and the `x-access-token:TOKEN@` URL form before anything is written.

Review feedback loop (Task 39) closes the loop on the review side. ForgeLoop persists `ReviewFeedback` rows (manual or imported from a `PullRequestReview`'s findings via an idempotent content hash) and `RevisionWorkItem` rows linking a feedback item to a workspace + branch and (optionally) an existing dev_task / subtask. A revision-work-item moves `proposed → approved` only when an `Approval` row of `target_type="revision_work_item"` exists. Task 39 records traceability and audit only — it never invokes OpenHands, git, or GitHub; revision execution flows through the existing Task 36/37/38 endpoints. **ForgeLoop still does not auto-merge, deploy, auto-approve PRs, sync live GitHub comments, or run autonomous feedback loops.** Task 40 will validate the complete build loop end-to-end.
